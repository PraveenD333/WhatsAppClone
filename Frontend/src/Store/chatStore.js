...
export const useChatStore = create((set, get) => ({
    ...
    sendMessage: async (formdata) => {
        ...
        if (conversation?.data?.length > 0) {
            const Conversation = conversation.data.find((conv) =>
            ...
            const updateConversations = state.conversation?.data?.map((con) => {
                if (con._id === message.conversation) {
                    return {
                        ...con,
                        lastMessage: message,
                        unreadCount: message?.receiver?._id === currentUser?._id ? (con.unreadCount || 0) + 1 : con.unreadCount || 0
                    }
                }
                return con;
            });
