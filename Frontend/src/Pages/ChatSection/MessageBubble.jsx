...
const MessageBubble = ({ message, theme, currentUser, onReact, deleteMessage }) => {
  ...
  const bubbleContentClass = isUserMessage ? `chat-bubble md:max-w-[30%] min-w-[100px] ${theme === "dark" ? "bg-[#25d36586] text-black" : "bg-[#25d36586] text-black"}` :
    `chat-bubble md:max-w-[50%] min-w-[130px] ${theme === "dark" ? "bg-[#ced2cf] text-black" : "bg-[#ced2cf] text-black"}`;
  ...
  return (
    <div className={`chat ${bubbleClass} mb-5`}>
      <div className={`${bubbleContentClass} relative group`}
        ref={messageRef}>
        <div className='flex justify-center gap-2 '>
          {message.contentType === "text" && <p className='mr-2 wrap-break-words'>{message.content}</p>}
          {message.contentType === "image" &&
            (
              <div>
                <img src={message.imageOrVideoUrl} alt="chat-media"
        ...
        <div className='self-end flex items-center justify-end gap-1 text-xs opacity-60 mt-2 ml-3'>
          <span>{format(new Date(message.createdAt), "hh:mm")}</span>
          {isUserMessage && (
            <>
              {message.messageStatus === "send" && <FaCheck size={12} />}
              {message.messageStatus === "delivered" && (<FaCheckDouble size={12} />)}
              {message.messageStatus === "read" && (<FaCheckDouble size={12} className='text-red-600' />)}
            </>
          )}
        </div>
        ...
        {showReactions && (
          <div
            ...
            <button className='hover:bg-[#fffff1a] rounded-full p-1'
        ...
        {message.reactions && message.reactions.length > 0 && (
          <div className={`absolute -bottom-2 ${isUserMessage ? "right-2" : "left-2"} ${theme === "dark" ? "bg-[#2a3942]" : "bg-gray-200"} rounded-full px-2 shadow-md`}>
        ...
        {showOptions && (
          <div ref={optionRef} className={`absolute top-8 rigth-1 z-50 w-36 rounded-xl shadow-lg py-2 text-sm ${theme === "dark" ? "bg-[#1d1f1f] text-white" : "bg-gray-100 text-black"} }`}>
            <button
              onClick={() => {
