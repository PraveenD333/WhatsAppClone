...
const StatusPreview = ({ contact, currentIndex, onPrev, onNext, currentUser, onClose, onDelete, theme, loading }) => {
    ...
    useEffect(() => {
        ...
        const interval = setInterval(() => {
            current += 2 //Increase Process by 2% event 100ms 50 steps = 5 Seconds
            setProgress(current)
            if (current >= 100) {
                ...
                <div className={`w-full h-full ${theme === 'dark' ? "bg-[#202c33]" : "bg-gray-800"} relative`}>
                    ...
                    <div className='w-full h-full flex items-center justify-center'>
                        {currentStatus.contentType === "text" ? (
                            ...
                            <img src={currentStatus.media} alt="image"
                                className='max-w-full max-h-full object-contain' />
                        ) : currentStatus.contentType === "video" ? (
                            <video src={currentStatus.media}
                                controls
                                autoPlay
                                className="max-w-full max-h-full object-contain" />
                        ) : null}
                    ...
                    {isOwner && (
                        <div className='absolute bottom-4 left-4 right-4'>
                            <button onClick={handleViewersToggle} className='flex items-center justify-between w-full text-white bg-black bg-opacity-50 rounded-lg px-4 py-2 hover:bg-opacity-70 transition-all'>
                                <div className='flex items-center space-x-2'>
                                            ...
                                            <div className='space-y-2'>
                                                {currentStatus?.viewers.map((viewer) => (
                                                    <div key={viewer?._id} className='flex items-center space-x-3'>
                                                        <img src={viewer?.profilePicture} alt={viewer?.username}
