...
const Status = () => {
  ...
  const { statuses, loading, error,
    fetchStatuses, createStatuses,
    viewStatus, deleteStatus,
    getUserStatuses, getOtherStatuses, clearError,
    initializeSocket, cleanupSocket } = useStatusStore();

  const userStatuses = getUserStatuses(user?._id)
  const otherStatues = getOtherStatuses(user?._id)
      ...
      <motion.div
        ...
        <div className='overflow-y-auto h-[calc(100vh-64px)]'>
          <div className={`flex p-3 space-x-4 shadow-md ${theme === 'dark' ? "bg-[rgb(17,27,33)]" : "bg-white"}`}>
                ...
                <>
                  <svg className='absolute top-0 left-0 w-12 h-12'
                    ...
                    {userStatuses.statuses.map((_, index) => {
                      ...
                      const segementLength = circumference / userStatuses.statuses.length;
                      const offset = index * segementLength;
                      return (
                        <circle
                          key={index}
                          cx='50'
                          ...
                          stroke='#25D366'
                          strokeWidth="4"
                          strokeDasharray={`${segementLength - 5} 5`}
            ...
            <div className='flex flex-col items-start flex-1'>
              ...
              <p className={`text-sm ${theme === 'dark' ? "text-gray-400" : "text-gray-500"}`}>
                {userStatuses ? `${userStatuses.statuses.length} status ${userStatuses?.statuses.length > 1 ? "." : ""} ${formatTimestamp(userStatuses.statuses[userStatuses.statuses.length - 1].timestamp)}` : "Tap to add status"}
              </p>
            </div>
            {userStatuses && (
          ...
          {showOption && userStatuses && (
            <div className={`p-2 shadow-md ${theme === 'dark' ? "bg-[rgb(17,27,33)]" : "bg-white"}`}>

              <button className='w-full text-left text-green-500 py-2 hover:bg-gray-100 px-2 rounded flex items-center'
                onClick={() => {
                  setShowCreateModel(true)
                  setShowOption(false)
                }}>
              ...
              <button className='w-full text-left text-blue-500 py-2 hover:bg-gray-100 px-2 rounded'
                onClick={() => {
          ...
          {!loading && otherStatues.length > 0 && (
            <div className={`p-4 space-y-4 shadow-md mt-4 ${theme === 'dark' ? "bg-[rgb(17,27,33)]" : "bg-white"}`}>
              ...
              {otherStatues.map((contact, index) => (
                <React.Fragment key={contact?.id}>
                  <StatusList
                    contact={contact}
                    onPreview={() => handleStatusPreview(contact)}
                    theme={theme} />
                  {index < otherStatues.length - 1 && (
          ...
          {!loading && statuses.length === 0 && (
            <div className='flex flex-col items-center justify-center p-8 text-center'>
              ...
              <p className={`text-sm ${theme === 'dark' ? "text-gray-500" : "text-gray-600"}}`}>
                Be the first to share a Status Update</p>
            </div>
          )}
        </div>
        {showCreateModel && (
            ...
            <div className={`p-6 rounded-lg max-w-md w-full mx-4 ${theme === 'dark' ? "bg-gray-800" : "bg-white"}`}>
              ...
              <div className='flex justify-end space-x-3'>
                ...
                <button
                  ...
                  className='px-4 py-2 bg-green-500 text-white font-semibold rounded hover:bg-green-300 disabled:opacity-50'>
